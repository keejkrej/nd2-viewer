...
- The primary workflow on this machine is Windows with MSVC; **Qt, VTK, and libCZI are supplied by [vcpkg](https://vcpkg.io/)** via the repo `vcpkg.json` manifest (not a standalone `C:\Qt\...` install).
...
- Warm up vcpkg dependencies (Qt, VTK, libczi): `.\scripts\install-vcpkg-deps.ps1` (re-run after `vcpkg.json` changes; first run can take a long time while VTK builds)
