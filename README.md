...
Qt, VTK, and libCZI are pulled in via [vcpkg](https://vcpkg.io/) using the repo `vcpkg.json` manifest. Install vcpkg yourself (on Windows, `scoop install vcpkg` is typical). The build scripts find it via `VCPKG_ROOT`, the Scoop install path, or `vcpkg` on `PATH`.
...
Install manifest dependencies once (or after changing `vcpkg.json`); the first run can take a long time while VTK builds:
