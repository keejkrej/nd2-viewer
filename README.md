...
Qt, VTK, ITK, and libCZI are pulled in via [vcpkg](https://vcpkg.io/) using the repo `vcpkg.json` manifest. Install vcpkg yourself; on Windows this repo expects a normal clone at `%USERPROFILE%\vcpkg` by default. The build scripts find it via `VCPKG_ROOT`, `%USERPROFILE%\vcpkg`, or `vcpkg` on `PATH`.

If you use a symlinked vcpkg location, use the helper scripts or point `VCPKG_ROOT` at the real directory. Qt rejects vcpkg build paths that go through symlinked directories.
...
- Release packages use the project version string, currently `0.1.8`, for example `dist\nd2-viewer-0.1.8-win64.exe`.
...
Run `windeployqt` from vcpkg's Qt (`vcpkg_installed\x64-windows\tools\Qt6\bin\windeployqt.exe`) so the build tree is runnable.
