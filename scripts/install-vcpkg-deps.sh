#!/usr/bin/env bash
# Warm up vcpkg manifest deps (Qt, VTK, libczi, ...) before build-macos.sh.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: ./scripts/install-vcpkg-deps.sh [options]
...
echo "vcpkg: installing manifest dependencies (Qt, VTK, libczi, ...) triplet=${vcpkg_triplet}"
